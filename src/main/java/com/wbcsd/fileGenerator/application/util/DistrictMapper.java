...
public class DistrictMapper {
	...
	public static String getOverriddenNameIfApplicable(String state, String districtName,
			List<String> masterDistricts) {
		if (explicitConversions.containsKey(districtName)) {
			return explicitConversions.get(districtName);
		}
		if (masterDistricts == null) {
			// System.err.println("Master districts empty In state: " + state +
			// " districk: " + districtName);
			return districtName;
		}
		...
		// System.err.println("In state: " + state + " district: " +
		// districtName + " not found in masterDistricts: "
		// + masterDistricts);
		return districtName;

	}

	public static Map<String, List<String>> getStateToDistrictMap() {
		CSVDataReader dataReader = new CSVDataReader();

		List<String[]> masterData = dataReader
				.readFileData("F:\\WBCSD\\fileGenerator\\src\\main\\resources\\NewInput\\DISTRICT_BDY.csv");
